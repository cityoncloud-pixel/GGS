...
| 主要输入 | `goal_seed.md` | `goal.md` |
| 主要输出 | `goal.md`（+ 评审与过程文件） | `plans/`、`reports/`、`specs/` 等 |
| 运行方式 | Cursor/Codex + `runner.prompt.md` | Cursor/Codex + harness 规则 + 适配器 |
...
```text
IDEA_CAPTURED
    ↓  Capture / Clarify（澄清问答）
CLARIFYING（逻辑态，可仍记为 IDEA_CAPTURED 并维护 open_questions）
    ↓  Draft
GOAL_DRAFTED
    ↓  Review
GOAL_REVIEWED
...
2. **启动 Runner**：将 `runner.prompt.md` 全文粘贴到 Codex/Cursor，或发送 [commandlist.md](../../commandlist.md) 中的 GGS 单句指令。
3. **配合澄清**：按 AI 提问在对话中回答，或写入 `goal_seed.md` 的 `## Q&A`；答不出可用 `UNKNOWN`。
4. **等待 Export**：直至 `state.json` 为 `EXPORTED` 且 `goal.review.json` 为 `PASS`，并生成 `goal.md`。
5. **进入 GAEH**：见 [GGS_GAEH_Handoff.md](./GGS_GAEH_Handoff.md)。
